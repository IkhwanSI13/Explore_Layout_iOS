//
//  AnotherTextFieldController.swift
//  explore_layout
//
...
class AnotherTextFieldController: UIViewController, UITextFieldDelegate {
    ...
    private func setupAnother(){
        ...
        with(setupTextFiled(placeholder: "Robust 0-9 only entry", yPos: 250)) {
            $0.tag = AnotherTextFieldController.DELEGATE_SIMPLE_FILTER
            $0.delegate = self
        }
        ...
        with(setupTextFiled(placeholder: "Reject centeral letters continous input", yPos: 350)) {
            $0.tag = AnotherTextFieldController.DELEGATE_COMPLEX_FILTER
            $0.delegate = self
        }
        ...
        with(setupTextFiled(placeholder: "Fixed number length without temp", yPos: 500)) {
            $0.tag = AnotherTextFieldController.DELEGATE_WHOLENUMBER_FILTER
            $0.delegate = self
        }
    }
        ...
        @objc private func wholeNumberFilterUndo(_ textField: UITextField) {
            if let text = textField.text,
                let number = Decimal(string: text.filter { $0.isWholeNumber }) {
                if (number <= Decimal(AnotherTextFieldController.MAX_VALUE)) {
                    lastValue = "\(number)"
                }
        ...
        func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
            ...
            case AnotherTextFieldController.DELEGATE_SIMPLE_FILTER:
               return (string.rangeOfCharacter(from: invalidCharacters) == nil)
            case AnotherTextFieldController.DELEGATE_COMPLEX_FILTER:
                return complexFilter(string, invalidCharacters, textField, range)
            case AnotherTextFieldController.DELEGATE_WHOLENUMBER_FILTER:
            ...
            if (string.rangeOfCharacter(from: invalidCharacters) == nil) {
                if let text = textField.text {
                    ...
                    if let number = Decimal(string: str.filter { $0.isWholeNumber }) {
                        if (number <= Decimal(AnotherTextFieldController.MAX_VALUE)) { textField.text = "\(number)" }
                        return false
                    }
                }
                return true
